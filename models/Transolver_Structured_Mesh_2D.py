...
from models.Physics_Attention import Physics_Attention_Structured_Mesh_2D
from models.MoE_class import MoE
...
class Transolver_block(nn.Module):  # 一个 transolver 块 ,网络架构见文章的图
    ...
    def __init__(
        ...
        self.MoE = MoE(hidden_dim, hidden_dim * mlp_ratio, output_dim=hidden_dim, num_experts=4, n_layers=0,top_k=2)
        # if self.last_layer:  #   如果是最后一层
...
class Model(nn.Module):
    ...
    def get_grid(self, batchsize=1):
        ...
        # pos大小 [B,H,W,ref*ref]代表原来的网格上每个点到ref*ref个网格点之间的距离
        return pos

    def forward(self, x, fx, T=None):
        if self.unified_pos:
            x = self.pos.repeat(x.shape[0], 1, 1, 1).reshape(x.shape[0], self.H * self.W, self.ref * self.ref)
            # 大小[N,H*W,ref*ref],扩展了位置信息。
            # 位置编码的作用相当于卷积核，把每个网格点都映射到ref*ref个网格点上，然后计算每个网格点到这些网格点的距离
        if fx is not None:
            fx = torch.cat((x, fx), -1) # fx:[B,H*W,1]
